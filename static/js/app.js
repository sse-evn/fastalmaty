        ...
        async function loadSettings() {
            try {
                ...
                document.getElementById('setting-kaspi_api_token').value = settings.kaspi_api_token || '';
        ...
        async function saveSettings() {
            const settings = {
                ...
                kaspi_api_token: document.getElementById('setting-kaspi_api_token').value
            };
            try {
                const res = await fetch('/api/settings', {
                    method: 'POST',
                ...
                localStorage.setItem('kaspi_api_token', settings.kaspi_api_token);
            } catch (err) {
...
function confirmQrDelivery() {
    ...
    fetch(`/api/order/scan-qr`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ order_id: currentScannedOrder.id })
    })
    .then(response => {
         if (!response.ok) {
    ...
    try {
        ...
        try {
            ...
            console.warn('Invalid JSON:', await res.text());
            data = [];
        }
        const orders = Array.isArray(data) ? data : (data.orders || []);
        const tbody = document.getElementById('courier-orders');
            ...
            orders.forEach(o => {
                rowsHtml += `
                    <tr>
                        ...
                        <td>
                            ${o.status === 'progress' ? 
                                `<button class="btn btn-success btn-sm" onclick="openQrScanner()">
                                    <i class="fas fa-qrcode"></i> Подтвердить QR
                                </button>` : 
                                `<span>${getStatusText(o.status)}</span>`
                            }
                        </td>
        ...
        function getKaspiAuthToken() {
            return document.getElementById('setting-kaspi_api_token').value || localStorage.getItem('kaspi_api_token') || '';
        }
        function importKaspiOrderById() {
            const orderId = document.getElementById('kaspi-order-id').value.trim();
            if (!orderId) {
                showNotification('Введите ID заказа', 'error');
                return;
            }
            showNotification('Поиск заказа в разработке...', 'info');
        }
        function importThisKaspiOrder(orderId) {
            showNotification('Импорт заказа ' + orderId + ' в разработке...', 'info');
        }